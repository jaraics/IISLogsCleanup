...
v1.03, 31/08/2015, Several changes to zip by day instead of last month
#>
...
# Default value for SavedDays (7 days)
$SavedDaysDefault = 7
...
$currentday = ($now).Day
if (! $SavedDays) {
    $SavedDays = $SavedDaysDefault
}
$delta = (Get-Date -Year $currentyear -Month $currentmonth -Day $currentday).AddDays(-$SavedDays)
...
$tmpstring = "Days delta: $delta"
Write-Host $tmpstring
Write-Logfile $tmpstring

#Fetch list of log files older than 1st day of previous month
$logstoremove = Get-ChildItem -Path "$($Logpath)\*.*" -Include *.log | Where {$_.CreationTime -lt $delta -and $_.PSIsContainer -eq $false}
...
$tmpstring = "Found $logcount logs earlier than $delta"
Write-Host $tmpstring
Write-Logfile $tmpstring
...
{
    $zipfilename = "$Logpath\$($date.Name).zip"
